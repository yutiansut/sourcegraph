...
const resultSetsQueryPart = ({ commit, repository, path, line, character }: Character) => gql`
...
const queryLocationReturningMethod = <M extends string>(method: M) => async ({
    ...
    const variables = { $repository: repository, $commit: commit, $path: path, $line: line, $character: character }
    const query = gql`
